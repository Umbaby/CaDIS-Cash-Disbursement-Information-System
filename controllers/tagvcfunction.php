<?php include "../models/functions.php";
    ...
    $anum = $_SESSION['anum'];
    $empid = isset($_SESSION['empid'])? mysqli_real_escape_string($conn,$_SESSION['empid']) :NULL;
    
    $notifications = $functions->view_notif_status($empid);
    ...
    if(isset($_POST['add']) && $_POST['add'] == "Add" && is_array($warrant)){
        ...
        if($warrant->num_rows==0){
            for($count = 0; $count < count($warrant); $count++){
                $net2 = str_replace(',', '', $net[$count]);
                $result = $functions->add_vc_et_at($date_encoded, $time_encoded, mysqli_real_escape_string($conn,$payee[$count]), $particular, $period_from, $period_to, $et_al, $ob_num, $gross, mysqli_real_escape_string($conn,$net2), $fund, $program, $remarks, $status, $payment_type, mysqli_real_escape_string($conn,$warrant[$count]), $date_r, $tagger);
    ...
    if(isset($v_select) && $v_select == "yes"){
        ...
        //$prows2 = $functions->view_profile2($empid);
